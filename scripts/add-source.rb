...
# Usage: ruby scripts/add-source.rb <relative/path/File.swift> [<PBXGroup path>]
...
path = ARGV[0]
group_path = ARGV[1]
abort 'usage: add-source.rb <File.swift> [group]' unless path
...
dir = group_path || File.dirname(path)
...
# 4. Sources build phase (app target Sources phase only — first one)
sources_marker = /(CC88E1DD2A6586E600E9387F \/\* Sources \*\/ = \{\n\t\t\tisa = PBXSourcesBuildPhase;\n\t\t\tbuildActionMask = 2147483647;\n\t\t\tfiles = \(\n)(.*?)(\n\t\t\t\);)/m
...
puts "added #{path} -> group #{dir} (ref #{id}, build #{build_id})"
