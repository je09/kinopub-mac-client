//
//  PlayerChromeView.swift
//  KinoPubAppleClient
//
//  The native AVKit view plus its window-chrome and display-sleep adapters (see
//  plans/refactor.md Phase 6). Kept in the app/platform UI layer — AVKit bridging is not part of
...
/// PiP, so all window changes remain scoped to the original host window. Also prevents display
/// sleep while playback is active in the focused window.
final class PlayerChromeView: AVPlayerView, AVPlayerViewPictureInPictureDelegate {
